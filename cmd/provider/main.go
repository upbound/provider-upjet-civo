...
import (
	...
	clusterapis "github.com/upbound/provider-civo/apis/cluster"
	namespacedapis "github.com/upbound/provider-civo/apis/namespaced"
	"github.com/upbound/provider-civo/config"
	resolverapis "github.com/upbound/provider-civo/internal/apis"
	"github.com/upbound/provider-civo/internal/clients"
	clustercontroller "github.com/upbound/provider-civo/internal/controller/cluster"
	namespacedcontroller "github.com/upbound/provider-civo/internal/controller/namespaced"
	"github.com/upbound/provider-civo/internal/features"
)
...
func main() {
	...
	kingpin.FatalIfError(resolverapis.BuildScheme(clusterapis.AddToSchemes), "Cannot register the cluster-scoped Civo APIs with the API resolver's runtime scheme")
	kingpin.FatalIfError(resolverapis.BuildScheme(namespacedapis.AddToSchemes), "Cannot register the namespaced Civo APIs with the API resolver's runtime scheme")
	log.Default().SetOutput(io.Discard)
	ctrl.SetLogger(zap.New(zap.WriteTo(io.Discard)))

	zl := zap.New(zap.UseDevMode(*debug))
	log := logging.NewLogrLogger(zl.WithName("provider-civo"))
	if *debug {
	...
	kingpin.FatalIfError(clusterapis.AddToScheme(scheme), "Cannot add cluster-scoped Civo APIs to scheme")
	kingpin.FatalIfError(namespacedapis.AddToScheme(scheme), "Cannot add namespaced Civo APIs to scheme")
	kingpin.FatalIfError(apiextensionsv1.AddToScheme(scheme), "Cannot register k8s apiextensions APIs to scheme")

	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		Scheme:           scheme,
		LeaderElection:   *leaderElection,
		LeaderElectionID: "crossplane-leader-election-provider-civo",
		Cache: cache.Options{
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(clustercontroller.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Civo controllers")
		kingpin.FatalIfError(namespacedcontroller.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Civo controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(clustercontroller.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Civo controllers")
		kingpin.FatalIfError(namespacedcontroller.Setup(mgr, namespacedOpts), "Cannot setup namespaced Civo controllers")
	}
